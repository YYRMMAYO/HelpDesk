...
所有请求都按解析出的**提交号**取内容而不是分支名，保证索引与文件出自同一份提交。
...
- 插件刚发布后的短时间内，分支名（`main`）在 CDN 上可能还是旧内容。宿主会先解析出当前的
  **提交号**并按提交号下载索引与文件——提交号不可变，因此索引与 DLL 永远来自同一份提交，
  不会出现「清单是新的、文件是旧的」这种装不上的情况。解析不到提交号时才退回分支名，
  此时靠下载重试兜底，仍失败会在提示里说明「过 1~2 分钟再试」。
